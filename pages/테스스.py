...
def classify_and_display(image, model):
    results = model(image)
    annotated_image = results[0].plot()

    st.image(annotated_image, channels="BGR", caption="재활용 분류 결과")
    ...
    for i, cls in enumerate(clss):
        conf = np.round(conff[i], 2) * 100
        if cls == 0:
            st.write(f'{i + 1}. :blue-background[종이팩] (약 {conf} %)')
        elif cls == 1:
            st.write(f'{i + 1}. :blue-background[종이컵] (약 {conf} %)')
        elif cls == 2:
            st.write(f'{i + 1}. :blue-background[종이컵+이물질] (약 {conf} %)')
        elif cls == 3:
            st.write(f'{i + 1}. :green-background[플라스틱] (약 {conf} %)')
        elif cls == 4:
            st.write(f'{i + 1}. :green-background[플라스틱+이물질] (약 {conf} %)')
        elif cls == 5:
            st.write(f'{i + 1}. :red-background[페트] (약 {conf} %)')
        elif cls == 6:
            st.write(f'{i + 1}. :red-background[페트+이물질] (약 {conf} %)')
        elif cls == 7:
            st.write(f'{i + 1}. :violet-background[페트+다중포장재] (약 {conf} %)')
        elif cls == 8:
            st.write(f'{i + 1}. :violet-background[페트+이물질+다중포장재] (약 {conf} %)')
